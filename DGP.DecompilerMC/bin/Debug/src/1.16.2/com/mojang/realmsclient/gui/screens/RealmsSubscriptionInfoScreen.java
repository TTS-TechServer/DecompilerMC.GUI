...
extends RealmsScreen {
    ...
    public void render(PoseStack poseStack, int n, int n2, float f) {
        ...
        this.font.draw(poseStack, SUBSCRIPTION_START_LABEL, (float)n3, (float)RealmsSubscriptionInfoScreen.row(0), 0xA0A0A0);
        this.font.draw(poseStack, this.startDate, (float)n3, (float)RealmsSubscriptionInfoScreen.row(1), 0xFFFFFF);
        if (this.type == Subscription.SubscriptionType.NORMAL) {
            this.font.draw(poseStack, TIME_LEFT_LABEL, (float)n3, (float)RealmsSubscriptionInfoScreen.row(3), 0xA0A0A0);
        } else if (this.type == Subscription.SubscriptionType.RECURRING) {
            this.font.draw(poseStack, DAYS_LEFT_LABEL, (float)n3, (float)RealmsSubscriptionInfoScreen.row(3), 0xA0A0A0);
        }
        this.font.draw(poseStack, this.daysLeft, (float)n3, (float)RealmsSubscriptionInfoScreen.row(4), 0xFFFFFF);
        super.render(poseStack, n, n2, f);
    }

    private Component daysLeftPresentation(int n) {
        if (n == -1 && this.serverData.expired) {
