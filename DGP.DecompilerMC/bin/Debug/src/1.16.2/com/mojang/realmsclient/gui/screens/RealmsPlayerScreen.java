...
extends RealmsScreen {
    ...
    extends ObjectSelectionList.Entry<Entry> {
        ...
        private void renderInvitedItem(PoseStack poseStack, PlayerInfo playerInfo, int n, int n2, int n3, int n4) {
            int n5 = !playerInfo.getAccepted() ? 0xA0A0A0 : (playerInfo.getOnline() ? 0x7FFF7F : 0xFFFFFF);
    ...
    extends RealmsObjectSelectionList<Entry> {
        ...
        public void itemClicked(int n, int n2, double d, double d2, int n3) {
            if (n2 < 0 || n2 > ((RealmsPlayerScreen)RealmsPlayerScreen.this).serverData.players.size() || RealmsPlayerScreen.this.hoveredUserAction == UserAction.NONE) {
                return;
            }
            if (RealmsPlayerScreen.this.hoveredUserAction == UserAction.TOGGLE_OP) {
                if (((RealmsPlayerScreen)RealmsPlayerScreen.this).serverData.players.get(n2).isOperator()) {
        ...
        public void selectItem(int n) {
            ...
            if (n != -1) {
                NarrationHelper.now(I18n.get("narrator.select", ((RealmsPlayerScreen)RealmsPlayerScreen.this).serverData.players.get(n).getName()));
            }
