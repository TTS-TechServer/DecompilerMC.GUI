...
extends RealmsScreen {
        ...
        if (n < this.pendingInvitationSelectionList.getItemCount()) {
            new Thread("Realms-reject-invitation"){
                ...
                public void run() {
                    try {
                        RealmsClient realmsClient = RealmsClient.create();
                        realmsClient.rejectInvitation(((Entry)((Entry)((RealmsPendingInvitesScreen)RealmsPendingInvitesScreen.this).pendingInvitationSelectionList.children().get((int)n))).pendingInvite.invitationId);
        ...
        if (n < this.pendingInvitationSelectionList.getItemCount()) {
            new Thread("Realms-accept-invitation"){
                ...
                public void run() {
                    try {
                        RealmsClient realmsClient = RealmsClient.create();
                        realmsClient.acceptInvitation(((Entry)((Entry)((RealmsPendingInvitesScreen)RealmsPendingInvitesScreen.this).pendingInvitationSelectionList.children().get((int)n))).pendingInvite.invitationId);
                        RealmsPendingInvitesScreen.this.minecraft.execute(() -> RealmsPendingInvitesScreen.this.updateList(n));
                    }
                    catch (RealmsServiceException realmsServiceException) {
    ...
    extends ObjectSelectionList.Entry<Entry> {
        ...
        private void renderPendingInvitationItem(PoseStack poseStack, PendingInvite pendingInvite, int n, int n2, int n3, int n4) {
            RealmsPendingInvitesScreen.this.font.draw(poseStack, pendingInvite.worldName, (float)(n + 38), (float)(n2 + 1), 0xFFFFFF);
            RealmsPendingInvitesScreen.this.font.draw(poseStack, pendingInvite.worldOwnerName, (float)(n + 38), (float)(n2 + 12), 0x6C6C6C);
            RealmsPendingInvitesScreen.this.font.draw(poseStack, RealmsUtil.convertToAgePresentationFromInstant(pendingInvite.date), (float)(n + 38), (float)(n2 + 24), 0x6C6C6C);
