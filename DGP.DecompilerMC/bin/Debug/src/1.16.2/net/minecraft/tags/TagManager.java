...
implements PreparableReloadListener {
    ...
    public CompletableFuture<Void> reload(PreparableReloadListener.PreparationBarrier preparationBarrier, ResourceManager resourceManager, ProfilerFiller profilerFiller, ProfilerFiller profilerFiller2, Executor executor, Executor executor2) {
        ...
        return ((CompletableFuture)CompletableFuture.allOf(completableFuture, completableFuture2, completableFuture3, completableFuture4).thenCompose(preparationBarrier::wait)).thenAcceptAsync(void_ -> {
            TagCollection<EntityType<?>> tagCollection;
            TagCollection<Fluid> tagCollection2;
            TagCollection<Item> tagCollection3;
            TagCollection<Block> tagCollection4 = this.blocks.load((Map)completableFuture.join());
            TagContainer tagContainer = TagContainer.of(tagCollection4, tagCollection3 = this.items.load((Map)completableFuture2.join()), tagCollection2 = this.fluids.load((Map)completableFuture3.join()), tagCollection = this.entityTypes.load((Map)completableFuture4.join()));
