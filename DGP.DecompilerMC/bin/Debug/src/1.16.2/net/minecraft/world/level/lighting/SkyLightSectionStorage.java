    ...
    protected void markNewInconsistencies(LayerLightEngine<SkyDataLayerStorageMap, ?> layerLightEngine, boolean bl, boolean bl2) {
        ...
        if (!this.sectionsToAddSourcesTo.isEmpty()) {
            ...
            while (longIterator.hasNext()) {
                ...
                if (n2 == 1) {
                    ...
                    n = SectionPos.sectionToBlockCoord(SectionPos.x(l));
                    ...
                    for (int i = 0; i < n5; ++i) {
                        ...
                        for (int j = 0; j < 16; ++j) {
                            for (int k = 0; k < 16; ++k) {
                                ...
                                switch (direction) {
                                    case NORTH: {
                                        l4 = BlockPos.asLong(n + j, n3 + k, n4);
                                        l3 = BlockPos.asLong(n + j, n3 + k, n4 - 1);
                                        break;
                                    }
                                    case SOUTH: {
                                        l4 = BlockPos.asLong(n + j, n3 + k, n4 + 16 - 1);
                                        l3 = BlockPos.asLong(n + j, n3 + k, n4 + 16);
                                        break;
                                    }
                                    case WEST: {
                                        l4 = BlockPos.asLong(n, n3 + j, n4 + k);
                                        l3 = BlockPos.asLong(n - 1, n3 + j, n4 + k);
                                        break;
                                    }
                                    default: {
                                        l4 = BlockPos.asLong(n + 16 - 1, n3 + j, n4 + k);
                                        l3 = BlockPos.asLong(n + 16, n3 + j, n4 + k);
                                    }
                                }
                                layerLightEngine.checkEdge(l4, l3, layerLightEngine.computeLevelFromNeighbor(l4, l3, 0), true);
                            }
                        }
                    }
                    for (int i = 0; i < 16; ++i) {
                ...
                for (n = 0; n < 16; ++n) {
                    for (n3 = 0; n3 < 16; ++n3) {
                        long l6 = BlockPos.asLong(SectionPos.sectionToBlockCoord(SectionPos.x(l)) + n, SectionPos.sectionToBlockCoord(SectionPos.y(l)) + 16 - 1, SectionPos.sectionToBlockCoord(SectionPos.z(l)) + n3);
        ...
        if (!this.sectionsToRemoveSourcesFrom.isEmpty()) {
            ...
            while (longIterator.hasNext()) {
                ...
                for (n2 = 0; n2 < 16; ++n2) {
                    for (n = 0; n < 16; ++n) {
                        long l7 = BlockPos.asLong(SectionPos.sectionToBlockCoord(SectionPos.x(l)) + n2, SectionPos.sectionToBlockCoord(SectionPos.y(l)) + 16 - 1, SectionPos.sectionToBlockCoord(SectionPos.z(l)) + n);
