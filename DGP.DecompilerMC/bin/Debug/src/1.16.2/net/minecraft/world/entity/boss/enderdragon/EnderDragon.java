...
implements Enemy {
    ...
    public void aiStep() {
        ...
        if (this.level.isClientSide) {
            ...
            if (!this.isSilent()) {
                f4 = Mth.cos(this.flapTime * ((float)Math.PI * 2));
                f3 = Mth.cos(this.oFlapTime * ((float)Math.PI * 2));
                if (f3 <= -0.3f && f4 >= -0.3f) {
        ...
        if (this.isDeadOrDying()) {
            f4 = (this.random.nextFloat() - 0.5f) * 8.0f;
            f3 = (this.random.nextFloat() - 0.5f) * 4.0f;
            float f5 = (this.random.nextFloat() - 0.5f) * 8.0f;
            this.level.addParticle(ParticleTypes.EXPLOSION, this.getX() + (double)f4, this.getY() + 2.0 + (double)f3, this.getZ() + (double)f5, 0.0, 0.0, 0.0);
            return;
        }
        this.checkCrystals();
        Vec3 vec3 = this.getDeltaMovement();
        f3 = 0.2f / (Mth.sqrt(EnderDragon.getHorizontalDistanceSqr(vec3)) * 10.0f + 1.0f);
        this.flapTime = this.phaseManager.getCurrentPhase().isSitting() ? (this.flapTime += 0.1f) : (this.inWall ? (this.flapTime += f3 * 0.5f) : (this.flapTime += (f3 *= (float)Math.pow(2.0, vec3.y))));
        ...
        if (this.level.isClientSide) {
            if (this.lerpSteps > 0) {
                ...
                d3 = this.getY() + (this.lerpY - this.getY()) / (double)this.lerpSteps;
                d2 = this.getZ() + (this.lerpZ - this.getZ()) / (double)this.lerpSteps;
                d = Mth.wrapDegrees(this.lerpYRot - (double)this.yRot);
                this.yRot = (float)((double)this.yRot + d / (double)this.lerpSteps);
                this.xRot = (float)((double)this.xRot + (this.lerpXRot - (double)this.xRot) / (double)this.lerpSteps);
                --this.lerpSteps;
                this.setPos(d4, d3, d2);
            ...
            if ((vec32 = dragonPhaseInstance.getFlyTargetLocation()) != null) {
                d3 = vec32.x - this.getX();
                d2 = vec32.y - this.getY();
                d = vec32.z - this.getZ();
                double d5 = d3 * d3 + d2 * d2 + d * d;
                float f6 = dragonPhaseInstance.getFlySpeed();
                double d6 = Mth.sqrt(d3 * d3 + d * d);
                if (d6 > 0.0) {
                    d2 = Mth.clamp(d2 / d6, (double)(-f6), (double)f6);
                }
                this.setDeltaMovement(this.getDeltaMovement().add(0.0, d2 * 0.01, 0.0));
                this.yRot = Mth.wrapDegrees(this.yRot);
                double d7 = Mth.clamp(Mth.wrapDegrees(180.0 - Mth.atan2(d3, d) * 57.2957763671875 - (double)this.yRot), -50.0, 50.0);
                ...
                f = (float)(2.0 / (d5 + 1.0));
                float f7 = 0.06f;
                this.moveRelative(0.06f * (f2 * f + (1.0f - f)), new Vec3(0.0, 0.0, -1.0));
                if (this.inWall) {
                ...
                this.setDeltaMovement(this.getDeltaMovement().multiply(d8, 0.91f, d8));
            }
        }
        ...
        float f8 = (float)(this.getLatencyPos(5, 1.0f)[1] - this.getLatencyPos(10, 1.0f)[1]) * 10.0f * ((float)Math.PI / 180);
        float f9 = Mth.cos(f8);
        float f10 = Mth.sin(f8);
        float f11 = this.yRot * ((float)Math.PI / 180);
        ...
        float f14 = Mth.sin(this.yRot * ((float)Math.PI / 180) - this.yRotA * 0.01f);
        float f15 = Mth.cos(this.yRot * ((float)Math.PI / 180) - this.yRotA * 0.01f);
        ...
        for (n = 0; n < 3; ++n) {
            ...
            float f17 = this.yRot * ((float)Math.PI / 180) + this.rotWrap(arrd2[0] - arrd[0]) * ((float)Math.PI / 180);
            float f18 = Mth.sin(f17);
            float f19 = Mth.cos(f17);
            f2 = 1.5f;
            f = (float)(n + 1) * 2.0f;
            this.tickPart(enderDragonPart, -(f12 * 1.5f + f18 * f) * f9, arrd2[1] - arrd[1] - (double)((f + 1.5f) * f10) + 1.5, (f13 * 1.5f + f19 * f) * f9);
        }
        if (!this.level.isClientSide) {
