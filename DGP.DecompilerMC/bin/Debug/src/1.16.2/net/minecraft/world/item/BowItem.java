...
implements Vanishable {
    ...
    public void releaseUsing(ItemStack itemStack, Level level, LivingEntity livingEntity, int n) {
        ...
        if (!level.isClientSide) {
            ...
            if ((n4 = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.POWER_ARROWS, itemStack)) > 0) {
                abstractArrow.setBaseDamage(abstractArrow.getBaseDamage() + (double)n4 * 0.5 + 0.5);
            }
            if ((n3 = EnchantmentHelper.getItemEnchantmentLevel(Enchantments.PUNCH_ARROWS, itemStack)) > 0) {
                abstractArrow.setKnockback(n3);
            }
