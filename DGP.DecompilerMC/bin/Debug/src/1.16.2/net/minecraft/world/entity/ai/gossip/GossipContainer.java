...
public class GossipContainer {
    ...
    public void transferFrom(GossipContainer gossipContainer, Random random, int n) {
        ...
        collection.forEach(gossipEntry -> {
            ...
            if (n >= 2) {
                this.getOrCreate(gossipEntry.target).entries.mergeInt((Object)gossipEntry.type, n, GossipContainer::mergeValuesForTransfer);
            }
        });
    }
    ...
    static class GossipEntry {
        ...
        public static DataResult<GossipEntry> load(Dynamic<?> dynamic) {
            return DataResult.unbox((App)DataResult.instance().group((App)dynamic.get("Target").read(SerializableUUID.CODEC), (App)dynamic.get("Type").asString().map(GossipType::byId), (App)dynamic.get("Value").asNumber().map(Number::intValue)).apply((Applicative)DataResult.instance(), GossipEntry::new));
        }
    }
}
