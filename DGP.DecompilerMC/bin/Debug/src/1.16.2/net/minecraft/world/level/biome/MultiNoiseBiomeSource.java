...
extends BiomeSource {
    ...
    static final class PresetInstance {
        public static final MapCodec<PresetInstance> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)ResourceLocation.CODEC.flatXmap(resourceLocation -> Optional.ofNullable(Preset.BY_NAME.get(resourceLocation)).map(DataResult::success).orElseGet(() -> DataResult.error((String)("Unknown preset: " + resourceLocation))), preset -> DataResult.success((Object)((Preset)preset).name)).fieldOf("preset").stable().forGetter(PresetInstance::preset), (App)RegistryLookupCodec.create(Registry.BIOME_REGISTRY).forGetter(PresetInstance::biomes), (App)Codec.LONG.fieldOf("seed").stable().forGetter(PresetInstance::seed)).apply((Applicative)instance, instance.stable((Object)((Function3)PresetInstance::new))));
