...
implements AutoCloseable {
    ...
    public CompletableFuture<Void> synchronize() {
        CompletionStage completionStage = this.submitTask(() -> Either.left(CompletableFuture.allOf((CompletableFuture[])this.pendingWrites.values().stream().map(pendingStore -> ((PendingStore)pendingStore).result).toArray(CompletableFuture[]::new)))).thenCompose(Function.identity());
