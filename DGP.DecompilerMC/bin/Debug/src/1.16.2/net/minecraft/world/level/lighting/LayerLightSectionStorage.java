...
extends SectionTracker {
    ...
    protected void markNewInconsistencies(LayerLightEngine<M, ?> layerLightEngine, boolean bl, boolean bl2) {
        ...
        if (!bl2) {
            ...
            while (longIterator.hasNext()) {
                long l4 = (Long)longIterator.next();
                this.checkEdgesForSection(layerLightEngine, l4);
            }
        }
        ...
        for (Direction direction : DIRECTIONS) {
            ...
            for (int i = 0; i < 16; ++i) {
                for (int j = 0; j < 16; ++j) {
                    ...
                    switch (direction) {
                        case DOWN: {
                            l4 = BlockPos.asLong(n + j, n2, n3 + i);
                            l3 = BlockPos.asLong(n + j, n2 - 1, n3 + i);
                            break;
                        }
                        case UP: {
                            l4 = BlockPos.asLong(n + j, n2 + 16 - 1, n3 + i);
                            l3 = BlockPos.asLong(n + j, n2 + 16, n3 + i);
                            break;
                        }
                        case NORTH: {
                            l4 = BlockPos.asLong(n + i, n2 + j, n3);
                            l3 = BlockPos.asLong(n + i, n2 + j, n3 - 1);
                            break;
                        }
                        case SOUTH: {
                            l4 = BlockPos.asLong(n + i, n2 + j, n3 + 16 - 1);
                            l3 = BlockPos.asLong(n + i, n2 + j, n3 + 16);
                            break;
                        }
                        case WEST: {
                            l4 = BlockPos.asLong(n, n2 + i, n3 + j);
                            l3 = BlockPos.asLong(n - 1, n2 + i, n3 + j);
                            break;
                        }
                        default: {
                            l4 = BlockPos.asLong(n + 16 - 1, n2 + i, n3 + j);
                            l3 = BlockPos.asLong(n + 16, n2 + i, n3 + j);
                        }
                    }
