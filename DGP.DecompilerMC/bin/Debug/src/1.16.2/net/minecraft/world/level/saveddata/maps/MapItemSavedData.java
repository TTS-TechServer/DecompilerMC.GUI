...
extends SavedData {
    ...
    public void tickCarriedBy(Player player, ItemStack itemStack) {
        ...
        for (int i = 0; i < this.carriedBy.size(); ++i) {
            ...
            object2 = ((HoldingPlayer)object3).player.getName().getString();
            if (object3.player.removed || !object3.player.inventory.contains(itemStack) && !itemStack.isFramed()) {
                ...
                this.decorations.remove(object2);
                continue;
            }
            if (itemStack.isFramed() || object3.player.level.dimension() != this.dimension || !this.trackingPosition) continue;
            this.addDecoration(MapDecoration.Type.PLAYER, object3.player.level, (String)object2, ((HoldingPlayer)object3).player.getX(), ((HoldingPlayer)object3).player.getZ(), object3.player.yRot, null);
        }
        if (itemStack.isFramed() && this.trackingPosition) {
            ...
            if (object2 != null && itemFrame.getId() != ((MapFrame)object2).getEntityId() && this.frameMarkers.containsKey(((MapFrame)object2).getId())) {
                this.decorations.remove("frame-" + ((MapFrame)object2).getEntityId());
            }
            ...
            this.frameMarkers.put(((MapFrame)object).getId(), (MapFrame)object);
        }
        if ((compoundTag = itemStack.getTag()) != null && compoundTag.contains("Decorations", 9)) {
            ...
            for (int i = 0; i < ((ListTag)object3).size(); ++i) {
                object = ((ListTag)object3).getCompound(i);
                if (this.decorations.containsKey(((CompoundTag)object).getString("id"))) continue;
                this.addDecoration(MapDecoration.Type.byIcon(((CompoundTag)object).getByte("type")), player.level, ((CompoundTag)object).getString("id"), ((CompoundTag)object).getDouble("x"), ((CompoundTag)object).getDouble("z"), ((CompoundTag)object).getDouble("rot"), null);
            }
        }
    }
