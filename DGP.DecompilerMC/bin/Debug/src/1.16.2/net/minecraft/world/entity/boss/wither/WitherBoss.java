...
RangedAttackMob {
    ...
    public void aiStep() {
        ...
        super.aiStep();
        for (n2 = 0; n2 < 2; ++n2) {
            this.yRotOHeads[n2] = this.yRotHeads[n2];
            this.xRotOHeads[n2] = this.xRotHeads[n2];
        }
        for (n2 = 0; n2 < 2; ++n2) {
            int n3 = this.getAlternativeTarget(n2 + 1);
            Entity entity2 = null;
            if (n3 > 0) {
            ...
            if (entity2 != null) {
                ...
                this.xRotHeads[n2] = this.rotlerp(this.xRotHeads[n2], f2, 40.0f);
                this.yRotHeads[n2] = this.rotlerp(this.yRotHeads[n2], f, 10.0f);
                continue;
            }
            this.yRotHeads[n2] = this.rotlerp(this.yRotHeads[n2], this.yBodyRot, 10.0f);
        }
        n2 = this.isPowered() ? 1 : 0;
        for (n = 0; n < 3; ++n) {
            double d = this.getHeadX(n);
            double d8 = this.getHeadY(n);
            double d9 = this.getHeadZ(n);
            this.level.addParticle(ParticleTypes.SMOKE, d + this.random.nextGaussian() * (double)0.3f, d8 + this.random.nextGaussian() * (double)0.3f, d9 + this.random.nextGaussian() * (double)0.3f, 0.0, 0.0, 0.0);
            if (n2 == 0 || this.level.random.nextInt(4) != 0) continue;
            this.level.addParticle(ParticleTypes.ENTITY_EFFECT, d + this.random.nextGaussian() * (double)0.3f, d8 + this.random.nextGaussian() * (double)0.3f, d9 + this.random.nextGaussian() * (double)0.3f, 0.7f, 0.7f, 0.5);
        }
