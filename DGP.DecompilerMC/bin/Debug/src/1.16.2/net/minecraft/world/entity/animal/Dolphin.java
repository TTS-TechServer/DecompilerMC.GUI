...
extends WaterAnimal {
    ...
    public void tick() {
        ...
        if (this.level.isClientSide && this.isInWater() && this.getDeltaMovement().lengthSqr() > 0.03) {
            ...
            float f2 = Mth.sin(this.yRot * ((float)Math.PI / 180)) * 0.3f;
    ...
    extends Goal {
        ...
        private void drop(ItemStack itemStack) {
            ...
            itemEntity.setDeltaMovement(0.3f * -Mth.sin(Dolphin.this.yRot * ((float)Math.PI / 180)) * Mth.cos(Dolphin.this.xRot * ((float)Math.PI / 180)) + Mth.cos(f2) * f3, 0.3f * Mth.sin(Dolphin.this.xRot * ((float)Math.PI / 180)) * 1.5f, 0.3f * Mth.cos(Dolphin.this.yRot * ((float)Math.PI / 180)) * Mth.cos(Dolphin.this.xRot * ((float)Math.PI / 180)) + Mth.sin(f2) * f3);
    ...
    extends MoveControl {
        ...
        public void tick() {
            ...
            double d4 = d3 * d3 + (d2 = this.wantedY - this.dolphin.getY()) * d2 + (d = this.wantedZ - this.dolphin.getZ()) * d;
            ...
            if (this.dolphin.isInWater()) {
                ...
                float f3 = -((float)(Mth.atan2(d2, Mth.sqrt(d3 * d3 + d * d)) * 57.2957763671875));
                f3 = Mth.clamp(Mth.wrapDegrees(f3), -85.0f, 85.0f);
                this.dolphin.xRot = this.rotlerp(this.dolphin.xRot, f3, 5.0f);
                float f4 = Mth.cos(this.dolphin.xRot * ((float)Math.PI / 180));
                float f5 = Mth.sin(this.dolphin.xRot * ((float)Math.PI / 180));
