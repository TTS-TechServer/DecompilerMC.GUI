...
extends QuadrupedModel<T> {
    ...
    public void setupAnim(T t, float f, float f2, float f3, float f4, float f5) {
        ...
        this.leg1.xRot = Mth.cos(f * 0.6662f * 0.6f + (float)Math.PI) * 0.5f * f2;
        this.leg2.zRot = Mth.cos(f * 0.6662f * 0.6f + (float)Math.PI) * 0.5f * f2;
        ...
        if (!((Entity)t).isInWater() && ((Entity)t).isOnGround()) {
            ...
            float f8 = 5.0f;
            this.leg2.yRot = Mth.cos(f6 * f * 5.0f + (float)Math.PI) * 8.0f * f2 * f7;
            this.leg2.zRot = 0.0f;
            this.leg3.yRot = Mth.cos(f6 * f * 5.0f) * 8.0f * f2 * f7;
            this.leg3.zRot = 0.0f;
            this.leg0.yRot = Mth.cos(f * 5.0f + (float)Math.PI) * 3.0f * f2;
            this.leg0.xRot = 0.0f;
