...
extends GuiComponent {
        ...
        if (!this.minecraft.options.hideGui) {
            ...
            if (this.title != null && this.titleTime > 0) {
                ...
                if ((n6 = Mth.clamp(n6, 0, 255)) > 8) {
                    ...
                    this.drawBackdrop(poseStack, font, -10, n4, 0xFFFFFF | n7);
                    font.drawShadow(poseStack, this.title, (float)(-n4 / 2), -10.0f, 0xFFFFFF | n7);
                    RenderSystem.popMatrix();
                    if (this.subtitle != null) {
                        RenderSystem.pushMatrix();
                        RenderSystem.scalef(2.0f, 2.0f, 2.0f);
                        n3 = font.width(this.subtitle);
                        this.drawBackdrop(poseStack, font, 5, n3, 0xFFFFFF | n7);
                        font.drawShadow(poseStack, this.subtitle, (float)(-n3 / 2), 5.0f, 0xFFFFFF | n7);
                        RenderSystem.popMatrix();
                    }
                    RenderSystem.disableBlend();
                    RenderSystem.popMatrix();
        ...
        for (MobEffectInstance mobEffectInstance : Ordering.natural().reverse().sortedCopy(collection)) {
            ...
            if (mobEffectInstance.isAmbient()) {
                ...
                if (mobEffectInstance.getDuration() <= 200) {
                    ...
                    f = Mth.clamp((float)mobEffectInstance.getDuration() / 10.0f / 5.0f * 0.5f, 0.0f, 0.5f) + Mth.cos((float)mobEffectInstance.getDuration() * (float)Math.PI / 5.0f) * Mth.clamp((float)n5 / 10.0f * 0.25f, 0.0f, 0.25f);
                }
            }
    ...
    protected void renderHotbar(float f, PoseStack poseStack) {
        ...
        for (n3 = 0; n3 < 9; ++n3) {
            ...
            this.renderSlot(n2, n, f, player, player.inventory.items.get(n3));
        }
        if (!itemStack.isEmpty()) {
            n3 = this.screenHeight - 16 - 3;
            if (humanoidArm == HumanoidArm.LEFT) {
                this.renderSlot(n4 - 91 - 26, n3, f, player, itemStack);
            } else {
                this.renderSlot(n4 + 91 + 10, n3, f, player, itemStack);
            }
        }
        if (this.minecraft.options.attackIndicator == AttackIndicatorStatus.HOTBAR && (f2 = this.minecraft.player.getAttackStrengthScale(0.0f)) < 1.0f) {
            ...
            int n8 = (int)(f2 * 19.0f);
            RenderSystem.color4f(1.0f, 1.0f, 1.0f, 1.0f);
            this.blit(poseStack, n, n2, 0, 94, 18, 18);
            this.blit(poseStack, n, n2 + 18 - n8, 18, 112 - n8, 18, n8);
        }
    ...
    public void renderSelectedItemName(PoseStack poseStack) {
        ...
        if (this.toolHighlightTimer > 0 && !this.lastToolHighlight.isEmpty()) {
            ...
            if (n > 0) {
                ...
                this.getFont().drawShadow(poseStack, mutableComponent, (float)n3, (float)n4, 0xFFFFFF + (n << 24));
                RenderSystem.disableBlend();
                RenderSystem.popMatrix();
            }
        }
    ...
    public void renderDemoOverlay(PoseStack poseStack) {
        ...
        this.getFont().drawShadow(poseStack, component, (float)(this.screenWidth - n - 10), 5.0f, 0xFFFFFF);
        this.minecraft.getProfiler().pop();
    }
    ...
    private void displayScoreboardSidebar(PoseStack poseStack, Objective objective) {
        ...
        this.getFont().getClass();
        int n5 = n4 * 9;
        int n6 = this.screenHeight / 2 + n5 / 3;
        int n7 = 3;
        int n8 = this.screenWidth - n2 - 3;
        int n9 = 0;
        int n10 = this.minecraft.options.getBackgroundColor(0.3f);
        int n11 = this.minecraft.options.getBackgroundColor(0.4f);
        for (Pair pair : arrayList) {
            ...
            this.getFont().getClass();
            int n13 = n6 - ++n9 * 9;
            int n14 = this.screenWidth - 3 + 2;
            this.getFont().getClass();
            Gui.fill(poseStack, n12 - 2, n13, n14, n13 + 9, n10);
            this.getFont().draw(poseStack, component2, (float)n12, (float)n13, -1);
            this.getFont().draw(poseStack, string, (float)(n14 - this.getFont().width(string)), (float)n13, -1);
            if (n9 != list.size()) continue;
            this.getFont().getClass();
            Gui.fill(poseStack, n12 - 2, n13 - 9 - 1, n14, n13 - 1, n11);
            Gui.fill(poseStack, n12 - 2, n13 - 1, n14, n13, n10);
            Font font = this.getFont();
            float f = n12 + n2 / 2 - n / 2;
            this.getFont().getClass();
            font.draw(poseStack, component, f, (float)(n13 - 9), -1);
        }
    }
    ...
    private void renderPlayerHealth(PoseStack poseStack) {
        ...
        for (n7 = 0; n7 < 10; ++n7) {
            ...
            n6 = n11 + n7 * 8;
            if (n7 * 2 + 1 < n20) {
                this.blit(poseStack, n6, n17, 34, 9, 9, 9);
            }
            if (n7 * 2 + 1 == n20) {
                this.blit(poseStack, n6, n17, 25, 9, 9, 9);
            }
            if (n7 * 2 + 1 <= n20) continue;
            this.blit(poseStack, n6, n17, 16, 9, 9, 9);
        }
        this.minecraft.getProfiler().popPush("health");
        for (n7 = Mth.ceil((f + (float)n14) / 2.0f) - 1; n7 >= 0; --n7) {
            n6 = 16;
            ...
            n4 = Mth.ceil((float)(n7 + 1) / 10.0f) - 1;
            n3 = n11 + n7 % 10 * 8;
            n2 = n13 - n4 * n16;
            if (n8 <= 4) {
                n2 += this.random.nextInt(2);
            }
            if (n19 <= 0 && n7 == n21) {
                n2 -= 2;
            }
            ...
            this.blit(poseStack, n3, n2, 16 + n5 * 9, 9 * n, 9, 9);
            if (bl) {
                if (n7 * 2 + 1 < n9) {
                    this.blit(poseStack, n3, n2, n6 + 54, 9 * n, 9, 9);
                }
                if (n7 * 2 + 1 == n9) {
                    this.blit(poseStack, n3, n2, n6 + 63, 9 * n, 9, 9);
                }
            }
            if (n19 > 0) {
                if (n19 == n14 && n14 % 2 == 1) {
                    this.blit(poseStack, n3, n2, n6 + 153, 9 * n, 9, 9);
                    --n19;
                    continue;
                }
                this.blit(poseStack, n3, n2, n6 + 144, 9 * n, 9, 9);
                n19 -= 2;
                continue;
            }
            if (n7 * 2 + 1 < n8) {
                this.blit(poseStack, n3, n2, n6 + 36, 9 * n, 9, 9);
            }
            if (n7 * 2 + 1 != n8) continue;
            this.blit(poseStack, n3, n2, n6 + 45, 9 * n, 9, 9);
        }
        ...
        if (n6 == 0) {
            ...
            for (n5 = 0; n5 < 10; ++n5) {
                ...
                n = n12 - n5 * 8 - 9;
                this.blit(poseStack, n, n4, 16 + n2 * 9, 27, 9, 9);
                if (n5 * 2 + 1 < n10) {
                    this.blit(poseStack, n, n4, n3 + 36, 27, 9, 9);
                }
                if (n5 * 2 + 1 != n10) continue;
                this.blit(poseStack, n, n4, n3 + 45, 27, 9, 9);
            }
            n18 -= 10;
        }
        this.minecraft.getProfiler().popPush("air");
        n5 = player.getAirSupply();
        n4 = player.getMaxAirSupply();
        if (player.isEyeInFluid(FluidTags.WATER) || n5 < n4) {
            n3 = this.getVisibleVehicleHeartRows(n6) - 1;
            n18 -= n3 * 10;
            n2 = Mth.ceil((double)(n5 - 2) * 10.0 / (double)n4);
            n = Mth.ceil((double)n5 * 10.0 / (double)n4) - n2;
