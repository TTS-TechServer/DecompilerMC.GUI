...
extends Screen {
    ...
    public void render(PoseStack poseStack, int n, int n2, float f) {
        ...
        for (n5 = 0; n5 < this.messages.length; ++n5) {
            string = this.messages[n5];
            if (string == null) continue;
            if (this.font.isBidirectional()) {
                string = this.font.bidirectionalShaping(string);
            }
            float f5 = -this.minecraft.font.width(string) / 2;
            this.minecraft.font.drawInBatch(string, f5, n5 * 10 - this.messages.length * 5, n6, false, matrix4f, bufferSource, false, 0, 0xF000F0, false);
            if (n5 != this.line || n7 < 0 || !bl2) continue;
            n4 = this.minecraft.font.width(string.substring(0, Math.max(Math.min(n7, string.length()), 0)));
            n3 = n4 - this.minecraft.font.width(string) / 2;
            if (n7 < string.length()) continue;
            this.minecraft.font.drawInBatch("_", n3, n9, n6, false, matrix4f, bufferSource, false, 0, 0xF000F0, false);
        }
        bufferSource.endBatch();
        for (n5 = 0; n5 < this.messages.length; ++n5) {
            string = this.messages[n5];
            if (string == null || n5 != this.line || n7 < 0) continue;
            ...
            int n12 = this.minecraft.font.width(string.substring(0, n3)) - this.minecraft.font.width(string) / 2;
            ...
            float f6 = n14;
            this.minecraft.font.getClass();
            bufferBuilder.vertex(matrix4f, f6, n9 + 9, 0.0f).color(0, 0, 255, 255).endVertex();
            float f7 = n15;
            this.minecraft.font.getClass();
            bufferBuilder.vertex(matrix4f, f7, n9 + 9, 0.0f).color(0, 0, 255, 255).endVertex();
