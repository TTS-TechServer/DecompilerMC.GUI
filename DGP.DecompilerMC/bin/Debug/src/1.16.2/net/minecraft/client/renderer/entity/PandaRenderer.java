...
extends MobRenderer<Panda, PandaModel<Panda>> {
    ...
    protected void setupRotations(Panda panda, PoseStack poseStack, float f, float f2, float f3) {
        ...
        if ((f5 = panda.getSitAmount(f3)) > 0.0f) {
            poseStack.translate(0.0, 0.8f * f5, 0.0);
            poseStack.mulPose(Vector3f.XP.rotationDegrees(Mth.lerp(f5, panda.xRot, panda.xRot + 90.0f)));
            poseStack.translate(0.0, -1.0f * f5, 0.0);
            if (panda.isScared()) {
                float f24 = (float)(Math.cos((double)panda.tickCount * 1.25) * Math.PI * (double)0.05f);
        ...
        if ((f4 = panda.getLieOnBackAmount(f3)) > 0.0f) {
            f6 = panda.isBaby() ? 0.5f : 1.3f;
            poseStack.translate(0.0, f6 * f4, 0.0);
            poseStack.mulPose(Vector3f.XP.rotationDegrees(Mth.lerp(f4, panda.xRot, panda.xRot + 180.0f)));
        }
    }
