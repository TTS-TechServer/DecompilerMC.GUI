...
extends ListModel<T> {
    ...
    public void setupAnim(T t, float f, float f2, float f3, float f4, float f5) {
        ...
        this.upperBodyParts[1].xRot = (0.065f + 0.05f * f6) * (float)Math.PI;
        ...
        this.upperBodyParts[2].xRot = (0.265f + 0.1f * f6) * (float)Math.PI;
        this.heads[0].yRot = f4 * ((float)Math.PI / 180);
        this.heads[0].xRot = f5 * ((float)Math.PI / 180);
    }

    @Override
    public void prepareMobModel(T t, float f, float f2, float f3) {
        for (int i = 1; i < 3; ++i) {
            this.heads[i].yRot = (((WitherBoss)t).getHeadYRot(i - 1) - ((WitherBoss)t).yBodyRot) * ((float)Math.PI / 180);
            this.heads[i].xRot = ((WitherBoss)t).getHeadXRot(i - 1) * ((float)Math.PI / 180);
        }
    }
