    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, double d, double d2, double d3) {
        ...
        for (BlockPos blockPos2 : BlockPos.betweenClosed(blockPos.offset(-10, -10, -10), blockPos.offset(10, 10, 10))) {
            ...
            if (longOpenHashSet.add(l)) {
                DebugRenderer.renderFloatingText(clientLevel.getChunkSource().getLightEngine().getDebugData(LightLayer.SKY, SectionPos.of(l)), SectionPos.x(l) * 16 + 8, SectionPos.y(l) * 16 + 8, SectionPos.z(l) * 16 + 8, 0xFF0000, 0.3f);
            }
            if (n == 15) continue;
