...
public class Font {
    ...
    private int drawInternal(String string, float f, float f2, int n, Matrix4f matrix4f, boolean bl, boolean bl2) {
        ...
        int n2 = this.drawInBatch(string, f, f2, n, bl, matrix4f, bufferSource, false, 0, 0xF000F0, bl2);
    ...
    private int drawInternal(FormattedCharSequence formattedCharSequence, float f, float f2, int n, Matrix4f matrix4f, boolean bl) {
        MultiBufferSource.BufferSource bufferSource = MultiBufferSource.immediate(Tesselator.getInstance().getBuilder());
        int n2 = this.drawInBatch(formattedCharSequence, f, f2, n, bl, matrix4f, (MultiBufferSource)bufferSource, false, 0, 0xF000F0);
        bufferSource.endBatch();
        return n2;
    }
    ...
    implements FormattedCharSink {
        ...
        public boolean accept(int n, Style style, int n2) {
            ...
            if (textColor != null) {
                ...
                f4 = (float)(n3 >> 16 & 0xFF) / 255.0f * this.dimFactor;
                f3 = (float)(n3 >> 8 & 0xFF) / 255.0f * this.dimFactor;
                f2 = (float)(n3 & 0xFF) / 255.0f * this.dimFactor;
            } else {
                f4 = this.r;
                f3 = this.g;
                f2 = this.b;
            ...
            if (!(bakedGlyph instanceof EmptyGlyph)) {
                ...
                f = this.dropShadow ? glyphInfo.getShadowOffset() : 0.0f;
                VertexConsumer vertexConsumer = this.bufferSource.getBuffer(bakedGlyph.renderType(this.seeThrough));
                Font.this.renderChar(bakedGlyph, bl, style.isItalic(), f6, this.x + f, this.y + f, this.pose, vertexConsumer, f4, f3, f2, f5, this.packedLightCoords);
            }
            float f7 = glyphInfo.getAdvance(bl);
            float f8 = f = this.dropShadow ? 1.0f : 0.0f;
            if (style.isStrikethrough()) {
                this.addEffect(new BakedGlyph.Effect(this.x + f - 1.0f, this.y + f + 4.5f, this.x + f + f7, this.y + f + 4.5f - 1.0f, 0.01f, f4, f3, f2, f5));
            }
            if (style.isUnderlined()) {
                this.addEffect(new BakedGlyph.Effect(this.x + f - 1.0f, this.y + f + 9.0f, this.x + f + f7, this.y + f + 9.0f - 1.0f, 0.01f, f4, f3, f2, f5));
            }
            this.x += f7;
            return true;
        }
