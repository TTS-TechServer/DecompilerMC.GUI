...
implements Widget {
    ...
    public void render(PoseStack poseStack, int n, int n2, float f) {
        ...
        int n6 = this.y0 + 4 - (int)this.getScrollAmount();
        if (this.renderHeader) {
            this.renderHeader(poseStack, n5, n6, tesselator);
        }
        this.renderList(poseStack, n5, n6, n, n2, f);
        this.minecraft.getTextureManager().bind(GuiComponent.BACKGROUND_LOCATION);
        ...
        tesselator.end();
        int n9 = this.getMaxScroll();
        if (n9 > 0) {
            int n10 = (int)((float)((this.y1 - this.y0) * (this.y1 - this.y0)) / (float)this.getMaxPosition());
            n10 = Mth.clamp(n10, 32, this.y1 - this.y0 - 8);
            int n11 = (int)this.getScrollAmount() * (this.y1 - this.y0 - n10) / n9 + this.y0;
            if (n11 < this.y0) {
                n11 = this.y0;
            }
            bufferBuilder.begin(7, DefaultVertexFormat.POSITION_TEX_COLOR);
            bufferBuilder.vertex(n3, this.y1, 0.0).uv(0.0f, 1.0f).color(0, 0, 0, 255).endVertex();
            bufferBuilder.vertex(n4, this.y1, 0.0).uv(1.0f, 1.0f).color(0, 0, 0, 255).endVertex();
            bufferBuilder.vertex(n4, this.y0, 0.0).uv(1.0f, 0.0f).color(0, 0, 0, 255).endVertex();
            bufferBuilder.vertex(n3, this.y0, 0.0).uv(0.0f, 0.0f).color(0, 0, 0, 255).endVertex();
            bufferBuilder.vertex(n3, n11 + n10, 0.0).uv(0.0f, 1.0f).color(128, 128, 128, 255).endVertex();
            bufferBuilder.vertex(n4, n11 + n10, 0.0).uv(1.0f, 1.0f).color(128, 128, 128, 255).endVertex();
            bufferBuilder.vertex(n4, n11, 0.0).uv(1.0f, 0.0f).color(128, 128, 128, 255).endVertex();
            bufferBuilder.vertex(n3, n11, 0.0).uv(0.0f, 0.0f).color(128, 128, 128, 255).endVertex();
            bufferBuilder.vertex(n3, n11 + n10 - 1, 0.0).uv(0.0f, 1.0f).color(192, 192, 192, 255).endVertex();
            bufferBuilder.vertex(n4 - 1, n11 + n10 - 1, 0.0).uv(1.0f, 1.0f).color(192, 192, 192, 255).endVertex();
            bufferBuilder.vertex(n4 - 1, n11, 0.0).uv(1.0f, 0.0f).color(192, 192, 192, 255).endVertex();
            bufferBuilder.vertex(n3, n11, 0.0).uv(0.0f, 0.0f).color(192, 192, 192, 255).endVertex();
            tesselator.end();
        }
