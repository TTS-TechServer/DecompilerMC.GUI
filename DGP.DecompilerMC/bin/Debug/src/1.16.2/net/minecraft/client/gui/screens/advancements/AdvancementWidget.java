...
extends GuiComponent {
    ...
    public void drawHover(PoseStack poseStack, int n, int n2, float f, int n3, int n4) {
        ...
        boolean bl2 = 113 - n2 - this.y - 26 <= 6 + n6 * 9;
        float f2 = this.progress == null ? 0.0f : this.progress.getPercent();
        int n7 = Mth.floor(f2 * (float)this.width);
        if (f2 >= 1.0f) {
            n7 = this.width / 2;
        ...
        int n9 = n2 + this.y;
        int n10 = bl ? n + this.x - this.width + 26 + 6 : n + this.x;
        int n11 = this.description.size();
        this.minecraft.font.getClass();
        int n12 = 32 + n11 * 9;
        if (!this.description.isEmpty()) {
            if (bl2) {
                this.render9Sprite(poseStack, n10, n9 + 26 - n12, this.width, n12, 10, 200, 26, 0, 52);
            } else {
                this.render9Sprite(poseStack, n10, n9, this.width, n12, 10, 200, 26, 0, 52);
            }
        }
        this.blit(poseStack, n10, n9, 0, advancementWidgetType3.getIndex() * 26, n7, 26);
        this.blit(poseStack, n10 + n7, n9, 200 - n8, advancementWidgetType2.getIndex() * 26, n8, 26);
        this.blit(poseStack, n + this.x + 3, n2 + this.y, this.display.getFrame().getTexture(), 128 + advancementWidgetType.getIndex() * 26, 26, 26);
        if (bl) {
            this.minecraft.font.drawShadow(poseStack, this.title, (float)(n10 + 5), (float)(n2 + this.y + 9), -1);
            if (string != null) {
        ...
        if (bl2) {
            for (int i = 0; i < this.description.size(); ++i) {
                float f3 = n10 + 5;
                this.minecraft.font.getClass();
                this.minecraft.font.draw(poseStack, this.description.get(i), f3, (float)(n9 + 26 - n12 + 7 + i * 9), -5592406);
            }
        } else {
            for (int i = 0; i < this.description.size(); ++i) {
                float f4 = n10 + 5;
                this.minecraft.font.getClass();
                this.minecraft.font.draw(poseStack, this.description.get(i), f4, (float)(n2 + this.y + 9 + 17 + i * 9), -5592406);
            }
        }
        this.minecraft.getItemRenderer().renderAndDecorateFakeItem(this.display.getIcon(), n + this.x + 8, n2 + this.y + 5);
    }
