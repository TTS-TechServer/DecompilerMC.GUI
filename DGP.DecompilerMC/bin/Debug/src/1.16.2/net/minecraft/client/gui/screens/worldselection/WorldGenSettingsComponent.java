...
Widget {
    ...
    public void init(final CreateWorldScreen createWorldScreen, Minecraft minecraft, Font font) {
        ...
        this.importSettingsButton = createWorldScreen.addButton(new Button(n, 185, 150, 20, new TranslatableComponent("selectWorld.import_worldgen_settings"), button -> {
            ...
            try {
                ...
                object4 = ServerResources.loadResources(packRepository.openAllSelected(), Commands.CommandSelection.INTEGRATED, 2, Util.backgroundExecutor(), minecraft);
                minecraft.managedBlock(object4::isDone);
                serverResources = ((CompletableFuture)object4).get();
            }
            ...
            object4 = RegistryReadOps.create(JsonOps.INSTANCE, serverResources.getResourceManager(), registryHolder);
            JsonParser jsonParser = new JsonParser();
            try {
                object3 = Files.newBufferedReader(Paths.get(string, new String[0]));
                object2 = null;
                try {
                    object = jsonParser.parse((Reader)object3);
                    dataResult = WorldGenSettings.CODEC.parse((DynamicOps)object4, object);
                }
            ...
            if (dataResult.error().isPresent()) {
                object3 = new TranslatableComponent("selectWorld.import_worldgen_settings.failure");
                object2 = ((DataResult.PartialResult)dataResult.error().get()).message();
                LOGGER.error("Error parsing world settings: {}", object2);
                object = new TextComponent((String)object2);
                minecraft.getToasts().addToast(SystemToast.multiline(minecraft, SystemToast.SystemToastIds.WORLD_GEN_SETTINGS_TRANSFER, (Component)object3, (Component)object));
            }
            serverResources.close();
            object3 = dataResult.lifecycle();
            dataResult.resultOrPartial(((Logger)LOGGER)::error).ifPresent(arg_0 -> this.lambda$null$6(minecraft, createWorldScreen, registryHolder, (Lifecycle)object3, arg_0));
        }));
    ...
    public void render(PoseStack poseStack, int n, int n2, float f) {
        ...
        if (this.preset.equals(Optional.of(WorldPreset.AMPLIFIED))) {
            this.font.getClass();
            this.amplifiedWorldInfo.renderLeftAligned(poseStack, this.typeButton.x + 2, this.typeButton.y + 22, 9, 0xA0A0A0);
        }
    }
