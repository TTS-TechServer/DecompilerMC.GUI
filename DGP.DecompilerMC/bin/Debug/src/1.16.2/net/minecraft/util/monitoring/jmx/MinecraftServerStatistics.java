...
implements DynamicMBean {
    ...
    private MinecraftServerStatistics(MinecraftServer minecraftServer) {
        ...
        MBeanAttributeInfo[] arrmBeanAttributeInfo = (MBeanAttributeInfo[])this.attributeDescriptionByName.values().stream().map(object -> ((AttributeDescription)object).asMBeanAttributeInfo()).toArray(MBeanAttributeInfo[]::new);
    ...
    public AttributeList getAttributes(String[] arrstring) {
        List<Attribute> list = Arrays.stream(arrstring).map(this.attributeDescriptionByName::get).filter(Objects::nonNull).map(attributeDescription -> new Attribute(((AttributeDescription)attributeDescription).name, ((AttributeDescription)attributeDescription).getter.get())).collect(Collectors.toList());
