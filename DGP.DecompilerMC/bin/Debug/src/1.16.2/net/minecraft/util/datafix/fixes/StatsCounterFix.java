            ...
            if (optional.isPresent()) {
                for (Map.Entry entry : ((Map)optional.get()).entrySet()) {
                    ...
                    if (CUSTOM_MAP.containsKey(string4)) {
                        string3 = "minecraft:custom";
                        string2 = CUSTOM_MAP.get(string4);
                    } else {
                        String string5;
                        ...
                        if ("stat.mineBlock".equals(string)) {
                            string3 = "minecraft:mined";
                            string2 = this.upgradeBlock(string4.substring(n + 1).replace('.', ':'));
                        } else if (ITEM_KEYS.containsKey(string)) {
                            string3 = ITEM_KEYS.get(string);
                            string5 = string4.substring(n + 1).replace('.', ':');
                            String string6 = this.upgradeItem(string5);
                            string2 = string6 == null ? string5 : string6;
                        } else {
                            if (!ENTITY_KEYS.containsKey(string)) continue;
                            string3 = ENTITY_KEYS.get(string);
                            string5 = string4.substring(n + 1).replace('.', ':');
                            string2 = ENTITIES.getOrDefault(string5, string5);
                        }
                    }
                    Dynamic dynamic3 = dynamic.createString(string3);
                    string = hashMap.computeIfAbsent(dynamic3, dynamic2 -> dynamic.emptyMap());
                    hashMap.put(dynamic3, string.set(string2, (Dynamic)entry.getValue()));
                }
            }
