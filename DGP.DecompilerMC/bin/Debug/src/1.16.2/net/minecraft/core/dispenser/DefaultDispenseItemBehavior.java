...
implements DispenseItemBehavior {
    ...
    public static void spawnItem(Level level, ItemStack itemStack, int n, Direction direction, Position position) {
        ...
        itemEntity.setDeltaMovement(level.random.nextGaussian() * (double)0.0075f * (double)n + (double)direction.getStepX() * d4, level.random.nextGaussian() * (double)0.0075f * (double)n + (double)0.2f, level.random.nextGaussian() * (double)0.0075f * (double)n + (double)direction.getStepZ() * d4);
